...
(6) $f(x)$在$E$上几乎处处等于零$\Leftrightarrow$$\int_{E}f(x)dx=0$ 

**※【定理】** 
若$f(x)$是$E$上的非负可积函数，则$f(x)$在$E$上是几乎处处有限的.

**※【命题】** 
若存在非负简单函数列单增趋近于非负可测函数，则L积分相等
...
**※【命题】** Beppo Levi非负渐升列积分定理
若存在非负可测函数列单增趋近于非负可测函数，则L积分相等
...
**※【推论】** 
$E_{k}\in \mathcal{M},E_{i}\cap E_{j}=\emptyset$. 若$f(x)$是$E=\cup_{i=1}^\infty E_{i}$上的非负可测函数，则$\int_{E}f(x)dx=\int_{\cup_{i=1}^\infty E_{i}}f(x)dx$. 

**※【定理】** Fatou 引理
设 $f_k(x) \in \mathcal{M}^+(E)$，则 $\int_E \liminf_{k \to \infty} f_k(x) dx \le \liminf_{k \to \infty} \int_E f_k(x) dx$.
设 $f_k(x) \in \mathcal{M}^+(E)$，存在一个非负可积函数$g\in L^1(E)$，使得对所有的$n$，都有$f_{n}(x)\leq g(x)$几乎处处成立，则 $\int_E \limsup_{k \to \infty} f_k(x) dx \ge \limsup_{k \to \infty} \int_E f_k(x) dx$.

例 Fatou引理的不等号是必要的
$f_{n}\in \mathcal{M}^+[0,1],f_{n}=\begin{cases}0 & x=0 \\ n & 0<x<1/n \\ 0 & 1/n\leq x\leq1\end{cases},\lim_{ n \to \infty }f_{n}(x)=0$
$\implies\int_E \liminf_{k \to \infty} f_k(x) dx < \liminf_{k \to \infty} \int_E f_k(x) dx$ 

**※【定理】** 非负可测函数可积的另一等价条件
$f(x)\in \mathcal{M}(E),f(x)a.e.$有限，$m(E)<\infty$，在$[0,\infty)$上做如下划分，$0=y_{0}<y_{1}<\dots<y_{k}<\dots\to \infty$，
其中$y_{k+1}-y_{k}<\delta$. 若令$E_{k}=\{ x\in E:y_{k}\leq f(x)<y_{k+1} \}$，则$f(x)$在$E$上是可积的当且仅当级数$\sum_{i=0}^\infty y_{i}m(E_{i})<+\infty$，此时有$\lim_{ \delta \to 0}\sum_{k=0}^\infty y_{k}m(E_{k})=\int_{E}f(x)dx$ 

例 $E\subset \mathbb{R},m(E)<\infty,f(x)\in \mathcal{M}^+(E)$，
则$f(x)$在$[0,\infty)$上可积的充分必要条件为$\sum_{n=0}^\infty m(\{ x\in E:f(x)\geq n \})>+\infty$.

例 $f(x)\in \mathcal{M}^+[a,b]$，
则$f^{2}(x)$在$[a,b]$上可积的充分必要条件为$\sum_{n=0}^\infty nm(\{ x\in E:f(x)\geq n \})<+\infty$. 
...
由定义容易知道下面的事实
1. 若$f\in L(E)$，$f(x)\,a.e.x\in E$有限
2. 若$E\in \mathcal{M}$，且$f(x)=0\,a.e.x\in E$，则$\int_{E}f(x)dx=0$ 
3. 若$f(x)\in \mathcal{M}(E),g\in L(E),|f(x)|\leq g(x)\,x\in E$，则$f\in L(E)$
4. $f\in L(\mathbb{R}^n)$，则$\lim_{ N \to \infty }\int_{\{ x\in \mathbb{R}^n,|x|\geq N \}}|f(x)|dx=0$ 
...
例 $g\in L(E),f_{n}\in L(E)$. 若$f_{n}(x)\geq g(x),a.e.x\in E$，则$\int_{E}\liminf_{ n \to \infty }f_{n}(x)dx\leq\liminf_{ n \to \infty }\int_{E}f_{n}(x)dx$ 

例 Jensen不等式
设$w\in \mathcal{M}^+(E),\int_{E}w(x)dx=1,\varphi(x)$是区间$I=[a,b]$上的下凸函数，$f\in \mathcal{M}(E),R(f)\subset I$. 若$fw\in L(E)$，则$\varphi\left( \int_{E}f(x)w(x)dx \right)\leq \int_{E}\varphi(f(x))w(x)dx$ 

**※【定义】** 凸函数
下凸函数：$f(\theta x_1 + (1 - \theta) x_2) \leq \theta f(x_1) + (1 - \theta) f(x_2)$ 
上凸函数：$f(\theta x_1 + (1 - \theta) x_2) \geq \theta f(x_1) + (1 - \theta) f(x_2)$ 

例 可积函数几乎处处为零的一种判别法
$f(x,y)\in L[a,b]$，若$\forall\,c\in[a,b]$，有$\int_{[a,c]}f(x)dx=0$，则$f(x)=0,a.e.x\in[a,b]$ 

例 设$g(x)$是$E$上的可测函数. 若对任意的$f\in L(E)$，都有$fg\in L(E)$，则除零测集$Z$外，$g(x)$是$E\setminus Z$上的有界函数

例 $f\in L(E),0<A=\int_{E}f(x)dx<\infty$，则存在$E$的可测子集$e$，使得$\int_{e}f(x)dx=\frac{A}{3}$
...
**※【定理】** Lebesgue控制收敛定理 LDCT
$f_{k}(x)\in L(E),\lim_{ k \to \infty }f_{k}(x)=f(x)\,a.e.x\in E$. 若存在$E$上的可积函数$F(x)$，使得$|f_{k}(x)|\leq F(x)\,a.e.x\in E$，则$\lim_{ k \to \infty }\int_{E}|f_{k}(x)-f(x)|dx=0\implies\lim_{ k \to \infty }\int_{E}f_{k}(x)dx=\int_{E}f(x)dx$ 

例 $f_{n}\in C^{(1)}(a,b),\lim_{ n \to \infty }f_{n}(x)=f(x),\lim_{ n \to \infty }f_{n}'(x)=F(x),x\in(a,b).$
若$f'(x),F(x)$在$(a,b)$上连续，则$f'(x)=F(x),x\in(a,b)$ 

**※【定理】** 依测度收敛型控制收敛定理
$f_{k}(x)\in L(E),f_{k}(x) \stackrel{ m}{ \longrightarrow}f(x)$. 若存在$E$上的可积函数$F(x)$，使得$|f_{k}(x)|\leq F(x)\,a.e.x\in E$，则$\lim_{ k \to \infty }\int_{E}f_{k}(x)dx=\int_{E}f(x)dx$ 
...
**※【定理】** 积分号下求导
$f(x,y)\in E\times(a,b),f(x,y)\in L(E)\cap D(a,b)$. 
若存在$F\in L(E)$，使得$\frac{|d}{dy}f(a,y)|\leq F(x)$，则$\frac{d}{dy}\int_{E}f(x,y)dx=\int_{E}\frac{d}{dy}f(x,y)dx$.
...
**※【定理】** 
若$f\in L(E)$，则$\forall\,\varepsilon>0,\exists\,\mathbb{R}^n$上具有紧支集的连续函数$g(x)$，使得$\int_{E}|f(x)-g(x)|dx<\varepsilon$.

**※【推论】** 
$f\in L(E)$，则$\exists\,\mathbb{R}^n$上具有紧支集的连续函数$\{ g_{k}(x) \}$，使得 (1) $\lim_{ k \to \infty }\int_{E}|f(x)-g_{k}(x)|dx=0$ (2) $\lim_{ k \to \infty }g_{k}(x)=f(x),a.e.x\in E$ 

例 $f\in L(\mathbb{R}^n)$. 若对一切$\mathbb{R}^n$上具有紧支集的连续函数$\varphi(x)$，有$\int_{\mathbb{R}^n}f(x)\varphi(x)dx=0$，则$f(x)=0,a.e.x\in \mathbb{R}^n$.

例 $f\in[a,b]$. 若对其支集在$(a,b)$内且可微的任意函数$\varphi(x)$，都有$\int_{[a,b]}f(x)\varphi'(x)dx=0$，则$f(x)=c,a.e.$ 

**※【定理】** 平均连续性
$f\in L(R^n)$，则有$\lim_{ h \to 0 }\int_{R^n}|f(x+h)-f(x)|dx=0$ 

例 若$E\subset R^n\cap \mathcal{M}$有界，则$\lim_{ |h| \to 0 }m(E\cap(E+\{ h \}))=m(E)$ 

**※【推论】** 
$f\in L(E)$，则存在具有紧支集的阶梯函数列$\{  |\varphi _{k}(x)|\}$，使得 (1) $\lim_{ k \to \infty }\varphi _{k}(x)=f(x),a.e.$ (2) $\lim_{ k \to \infty }\int_{E}|f(x)-\varphi(x)|dx=0$.

**※【定理】** Riemann-Lebesgue引理
设函数 $f(x)$ 在区间 $[a, b]$ 上可积，则有以下结论：$\lim_{k \to \infty} \int_a^b f(x) \cos(kx) , dx = 0$ $\lim_{k \to \infty} \int_a^b f(x) \sin(kx) , dx = 0$  直观上，这意味着当被积函数的振荡频率 **k** 趋于无穷大时，其积分值趋于零。这是因为快速振荡的正弦或余弦函数在积分过程中会相互抵消

例 Riemann-Lebesgue引理的推广
$\{ g_{n}(x) \}$是$[a,b]$上的可测函数列，且满足 (1) $|g(x)|\leq M$  (2) $\forall\,c\in[a,b],\lim_{ n \to \infty }\int_{[a,c]}g_{n}(x)dx=0$，则$\forall\,f\in L[a,b]$，有$\lim_{ n \to \infty }\int_{[a,b]}f(x)g_{k}(x)dx=0$.

例 $\{ \lambda _{n} \}\in \mathbb{R}$，且$\lambda _{n}\to \infty$，则$m(\{ x\in \mathbb{R}:\lim_{ n \to \infty }\sin\lambda _{n}x \text{存在}\})=0$. 

例 $f(x)\in \mathcal{M}[0,1]$有界. 若$I_{n}=\int_{[0,1]}x^nf(x)dx=0$，则$f(x)=0,a.e.x\in[0,1]$. 
...
设 $f$ 在 $[a,b]$ 上有界，$M(x) = \limsup_{y \to x} f(y)$，$m(x) = \liminf_{y \to x} f(y)$，则 $M(x), m(x)$ 可测，且 $m(x) \le f(x) \le M(x)$，$f$ 在 $[a,b]$ 上 Riemann 可积 $\iff M(x)=m(x)$ a.e.. 
也可以表达成不连续点集是零测集.  $f$ 在 $[a,b]$ 上有界，$f \in R[a,b] \iff f$ 在 $[a,b]$ 上几乎处处连续.
...
**※【定理】** 
$\{ E_{k} \}$递增可测集列，其并集是$E$，又$f\in L(E_{k})$. 若极限$\lim_{ k \to \infty }\int_{E_{k}}|f(x)|dx$存在，则$f\in L(E)$，且有$\int_{E}f(x)dx=\lim_{ k \to \infty }\int_{E_{k}}f(x)dx$. 
...
例 $f\in L[0,\infty),a>0$，则$\int_{0}^\infty \sin axdx\int_{0}^{\infty}f(y)\exp(-xy)dy=a\int_{0}^{\infty} \frac{f(y)y}{a^{2}+y^{2}}$.

例 $\int_{0}^\infty \exp(-x^{2})dx=\frac{\sqrt{ \pi }}{2}$.
### 2 积分的几何意义
...
例 L中无卷积单位
$L(\mathbb{R})$中不存在函数$u(x)$，使得对一切$f\in L(\mathbb{R})$，有$(u*f)(x)=f(x),a.e.$.

**※【定义】** 分布函数
$f\in \mathcal{M}(E),f_{*}(\lambda)=m(\{ x\in E:|f(x)|>\lambda \}),\lambda>0$.
