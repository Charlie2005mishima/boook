...
显然有$D^+f(x_{0})\geq D_+f(x_{0}),D^-f(x_{0})\geq D_-f(x_{0})$ 
...
例 设$E\subset(a,b)$，且$m(E)=0$，我们可以做一个在$[a,b]$上连续且递增的函数$f(x)$，使得$f'(x)=+\infty,x\in E$  $f_{n}(x)=m([a,x]\cap G_{n}),f(x)=\sum_{i=1}^\infty f_{n}(x)$.
...
例 在$[0,1]$上定义函数$f(x)=\begin{cases}x\sin \frac{\pi}{x} & 1\geq x>0 \\ 0 & x=0\end{cases}$，则$f(x)$不是有界变差函数.

由定义我们可得到下属简单事实、
1. 设$f\in BV[a,b]$，则$f(x)$在$[a,b]$上是有界函数
2. $BV[a,b]$构成一个线性空间
...
其中$g(x)=\frac{1}{2}\bigvee_{a}^x(f)+\frac{1}{2}f(x),h(x)=\frac{1}{2}\bigvee_{a}^x(f)-\frac{1}{2}f(x)$ 

例 若$f\in BV[a,b]$，则$f(x)$几乎处处可微，且$\frac{d}{dx}(\bigvee_{a}^x(f))=|f'(x)|,a.e.x\in[a,b]$ 
...
**※【推论】** 
若$f\in L[a,b]$，则对$[a,b]$中几乎处处的点$x$，都有$\lim_{ h \to 0 } \frac{1}{h}\int_{0}^h|f(x+t)-f(x)|dt=0$ 我们成满足上式的点为$f(x)$的Lebesgue点

例 设$f\in L(\mathbb{R})$. 若对区间$[a,b]$，有$\int_{a}^b|f(x+h)-f(x)|dx=o(|h|)(h\to0)$，则$f(x)=C,a.e.$ 

例 设$f\in L[a,b]$且令$F(x)=\int_{a}^xf(t)dt$，则$F\in BV[a,b]$，且$\bigvee_{a}^b(F)\leq \int_{a}^b|f(x)|dx$ 
