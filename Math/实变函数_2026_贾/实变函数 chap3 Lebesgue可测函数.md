...
$m(E)<\infty$不满足产生的反例 $f_{n}(x)=\chi_{(0,n)}(x),f_{n}\to 1,a.e.;$
...
(i) Riesz定理：若$f_{n} \stackrel{ m}{ \longrightarrow}f$，则$\{ f_{n} \}_{n}$中必有子列几乎处处收敛于$f$. 
(ii) Lebesgue定理：若$m(D)<\infty$ 并且$f_{n}(x)\to f(x),a.e.x\in D$，则$f_{n} \stackrel{ m}{ \longrightarrow}f$.

**※【定理】** 依测度收敛的极限唯一
若$f_{k} \stackrel{ m}{ \longrightarrow}f,f_{k} \stackrel{ m}{ \longrightarrow}g$，则$f(x)=g(x),a.e.$

**※【定理】** 
$f_{k}(x)$是$E$上几乎处处有限的可测函数列. 若$\forall\,\delta>0,\exists\,E_{\delta}\subset E,m(E_{\delta})<\delta,s.t.f_{k}\rightrightarrows f\,E\setminus E_{\delta}$ 
...
**※【定理】** Lusin定理 Лузин
设 $f$ 是定义在可测集 $D\subset \mathbb{R}^n$ （ $m(D)<\infty$ ）上的**几乎处处有限的可测函数**. 
则对 $\forall\,\varepsilon>0$，存在**闭集** $F\subset D$ ，使得：
